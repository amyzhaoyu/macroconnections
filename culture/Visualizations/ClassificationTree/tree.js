...
$.get(url,{}, function (d) { 
  ...
  subset = data.slice(0,10);
  cats = _.groupBy(data, 'category');
  cat_list = [];
  for (var key in cats){ // category
    inds = _.groupBy(cats[key], 'industry'); 
    ind_list = [];
    for (var k in inds){ //industry
      occs = _.groupBy(inds[k], 'occupation');
      occ_list = [];
      for(var o in occs){
        raw_list = []
